...
trait ActorMap extends Actor {
  ...
  private def getOrMake(id: String) = actors get id getOrElse {
    println(s"creating actor $id <================================")
     val actorstr = context.actorOf(Props(mkActor(id)), name = id) ~ { actor =>
      actors += (id -> actor)
      context watch actor
    }
    println(actorstr.path.toString + "____________________________________________--")
    actorstr
  }
}
